...
### 视频生成是异步的——agent 怎么管理长时间运行的任务？

提交任务后，提供者需要 30 秒到几分钟处理。如果 agent 阻塞等待，用户体验会很差——"agent 卡住了"。

OpenClaw 用**后台作业队列**模式解决：agent 调用 `video_generate`，OpenClaw 立即返回任务 id 而非阻塞等待。提供者在后台处理，完成后通过内部事件唤醒同一会话。这跟 Sidekiq/Celery 的作业模式是一个思路——提交即返回，完成再通知。

agent 在等待期间可以继续响应用户："视频正在生成，大概 2 分钟"。这比"agent 卡住了 2 分钟没反应"好得多。

### 三种运行时模式为什么按输入媒体递进？

OpenClaw 把视频生成拆成三种模式：`generate`（文本→视频）、`imageToVideo`（图像→视频）、`videoToVideo`（视频→视频）。这不是并列关系，而是**按输入媒体递进**——输入越复杂，模式越高级。

16 个提供者能力各异：DeepInfra 只支持 `generate`，fal 支持全部三种。工具在提交前验证活跃模式——如果用户给了视频参考但当前提供者不支持 `videoToVideo`，直接拒绝而非提交后失败。这跟 HTTP 的 `Accept` 头协商是一个道理：客户端说"我要 video/mp4"，服务器不支持就返回 406 Not Acceptable，不是假装支持然后给个损坏的文件。

路由逻辑因此可以自动化：用户给文本就走 `generate`，给图像就走 `imageToVideo`，给视频就走 `videoToVideo`。然后在支持该模式的提供者中选最优的。

### 会话内去重——为什么不是全局去重？

同一会话中的重复 `video_generate` 调用返回当前任务状态而非启动新生成。这避免了用户连续点击"生成"导致的重复提交——类似数据库的幂等写入，相同会话、相同意图只产生一个任务。

但**跨会话不限制**。你可以在不同会话中同时生成多个视频。这是有意为之：不同会话可能代表不同任务、不同用户，全局去重会引入跨会话状态管理和锁竞争，复杂度远超收益。会话级去重够用——同一用户在同一会话里不会真的想同时生成 5 个相同视频。
