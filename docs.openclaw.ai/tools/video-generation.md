...
> **架构精读**
>
> **问题**：视频生成是异步的——提交任务后，提供者需要 30 秒到几分钟处理。OpenClaw 怎么在 agent 会话中管理这种长时间运行的任务，又怎么在 16 个能力各异的提供者间做路由？
>
> **类比：后台作业队列 + 能力协商**。类似 Sidekiq/Celery 的作业模式：agent 调用 `video_generate`，OpenClaw 立即返回任务 id 而非阻塞等待。提供者在后台处理，完成后通过内部事件唤醒同一会话。这让 agent 可以继续响应用户，而非挂起等待。能力协商则类似 HTTP 的 `Accept` 头——每个提供者支持不同的模式子集（`generate`/`imageToVideo`/`videoToVideo`），工具在提交前验证活跃模式，不支持的请求直接拒绝而非提交后失败。
>
> **关键洞察**：三种运行时模式不是并列的，而是按输入媒体递进。`generate` 无参考媒体，`imageToVideo` 有图像参考，`videoToVideo` 有视频参考。提供者可以支持任意子集——比如 DeepInfra 只支持 `generate`，fal 支持全部三种。这种设计让路由逻辑可以根据用户输入自动选择：用户给文本就走 `generate`，给图像就走 `imageToVideo`，给视频就走 `videoToVideo`，然后在支持该模式的提供者中选择。
>
> **会话内去重**：当作业在进行中时，同一会话中的重复 `video_generate` 调用返回当前任务状态而非启动新生成。这避免了用户连续点击导致的重复提交——类似数据库的幂等写入，相同会话、相同意图只产生一个任务。跨会话则不限制，你可以同时在不同会话中生成多个视频。
