...
> **架构精读**
>
> **问题**：14 个搜索提供者，有的返回结构化摘要，有的返回 AI 合成答案带引用，有的用神经搜索。OpenClaw 怎么在它们之间做路由，又怎么让 agent 不用关心底层差异？
>
> **类比：数据库查询优化器的多后端适配**。PostgreSQL 查询优化器不关心数据在 SSD、HDD 还是远程存储——它只看统计信息和成本模型选执行计划。OpenClaw 的 `web_search` 做类似的事：agent 只说"我要搜 X"，OpenClaw 根据配置的提供者优先级、认证状态、结果风格（摘要 vs AI 合成 vs 密集摘录）选择后端。结果统一成 agent 可消费的结构化格式，不管底层是 Brave 的 snippet、Gemini 的 AI 答案、还是 Parallel 的 dense excerpt。
>
> **关键洞察**：搜索和浏览器是两条独立路径。`web_search` 是轻量级 HTTP 工具——调 API、拿结果、缓存 15 分钟。`browser` 是完整浏览器自动化——启动 Chrome、渲染 JS、交互页面。大部分搜索场景（找文档、查 API、看新闻）走 `web_search` 就够了；只有 JS 重的站点或需要登录的页面才需要 `browser`。这种分离让 agent 能用最便宜的方式解决 80% 的信息获取需求。
>
> **缓存策略**：查询级缓存，15 分钟 TTL。同一查询短时间内多次调用直接返回缓存结果，不重复调 API。这降低了延迟和成本，但也意味着搜索结果有最多 15 分钟延迟。实时性要求高的场景（股票价格、突发新闻）需要 agent 自己判断是否绕过缓存或换用其他工具。
