...
### 结构化摘要 vs AI 合成答案——什么时候该选前者？

MiniMax 返回类似早期搜索引擎的结果——标题、URL、摘要、相关查询。AI 合成提供者（Gemini/Grok/Kimi）返回"我帮你总结好了"的答案。

这跟目录索引 vs 百科全书是一个思路。目录索引给你链接列表，你自己点进去读；百科全书直接给你总结好的条目。前者灵活（你可以选择读哪些、怎么组合），后者高效（一次拿到答案）。

选择逻辑：
- **需要 agent 自主判断、多轮搜索迭代**（"调研 X 技术的现状，对比 Y 和 Z"）→ MiniMax，给 agent 选择权
- **需要一次性拿到答案**（"PostgreSQL 的 vacuum 怎么工作"）→ AI 合成提供者，省掉 agent 的二次处理

### 区域过滤——为什么对中文搜索有价值？

MiniMax 支持区域过滤：`global`（全球）和 `cn`（中国大陆）。搜中文内容时用 `cn` 区域能拿到更相关的中文结果——MiniMax 的索引对中国大陆网站覆盖更全，排名算法对中文查询优化更好。

搜英文内容时用 `global` 区域——国际网页覆盖更全。

这跟 Google 的区域过滤（`gl=cn` vs `gl=us`）是一个思路。不同区域的搜索引擎索引不同，排名算法也不同。选对区域能显著提高结果相关性。
