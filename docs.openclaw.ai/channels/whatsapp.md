...
> ## [标签页] Access control and activation
...
> ## [展开] Message normalization and context
...
回复元数据字段也会在可用时填充（`ReplyToId`、`ReplyToBody`、`ReplyToSender`、发送者 JID/E.164）。当引用回复目标是可下载的媒体时，OpenClaw 通过正常入站媒体存储保存它，并暴露为 `MediaPath`/`MediaType`，使 agent 可以检查引用的图片，而不仅仅看到 `<media:image>`。
...
> ## [展开] Delivery, chunking, and media
...
> ## [展开] Multi-account and credentials
...
> ## [展开] Tools, actions, and config writes
...
当目标账号没有活跃的网关监听器时，出站发送会快速失败。确认 Gateway 正在运行且账号已链接。
...
- 仅当你希望该作用域允许所有群时，才添加通配符群 `systemPrompt`。如果仍希望只允许固定的群 ID 集合，不要用 `groups["*"]` 作为提示词默认值，而是在每个明确允许入群组条目上重复提示词。
- 群准入和发送者授权是独立检查。`groups["*"]` 扩大了能到达群处理的群集合，但不自动授权这些群中的每个发送者。发送者访问仍由 `channels.whatsapp.groupPolicy` 和 `channels.whatsapp.groupAllowFrom` 分别控制。
