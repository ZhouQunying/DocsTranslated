...
> 本节提炼进度草稿的设计模式。跳过不影响阅读后续翻译。

### 核心问题

Agent 执行工具调用可能要几十秒。用户这段时间看到的是沉默。发"正在处理..."之类的临时消息又会堆满聊天记录。

### 一条可变消息模式

解法是**只创建一条消息，反复更新它的内容**：

1. Agent 开始干活 → 创建一条草稿消息（"正在思考..."）
2. 调工具 → 更新草稿（"🔎 搜索中..."）
3. 调下一个工具 → 再更新（"🛠️ 跑测试..."）
4. 生成完成 → 草稿变成最终答案（或删掉草稿、发新消息）

用户看到的是一条消息在"跳动"，而不是一堆临时消息。

### 延迟激活

草稿不是一开始就创建的——等 Agent 确认在干活（`activationThresholdMs` + 至少一个工具调用）才创建。这避免了简单问题也弹出"正在思考..."的尴尬。

### 类型化工具进度

每种工具调用有对应的进度图标和标签（📖 读文件、🔎 搜索、🛠️ 跑命令）。标签池（`labelPool`）限制同时显示的行数——超过就折叠旧的，保持草稿简洁。

### 收尾策略

通道能编辑消息时（Telegram、Discord），草稿直接**原地变成最终答案**——用户看到的那条消息从"正在搜索..."平滑切换到完整回复。不能编辑时，删掉草稿、发一条新消息。
