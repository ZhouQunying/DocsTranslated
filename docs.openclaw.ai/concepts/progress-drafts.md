...
### Agent 在想，用户在等——怎么不让人觉得卡？

Agent 跑工具调用可能要几十秒。这段时间用户看到的是：没反应。发一堆"正在处理..."的临时消息？聊天记录就塞满垃圾了。

关键直觉：**只用一条消息，反复改它的内容**。像终端里的进度条——位置不动，内容在变。用户看到的是一条消息在"跳动"（搜索中… → 跑测试… → 最终回复），而不是一堆临时消息。

### 为什么不一开始就显示？

简单问题（"今天周几？"）半秒就答完了，弹个"正在思考..."再消失反而尴尬。所以草稿有个**延迟激活**——等 Agent 确认在干活（超过阈值时间 + 至少一个工具调用）才创建。短回答用户根本看不到草稿的存在。

### 收尾时的分叉

通道能编辑消息（Telegram、Discord）时，草稿直接**原地变成最终回复**——用户看到那条消息从"正在搜索…"平滑切换到答案，感觉像打字机。

不能编辑时（比如旧版 API），删掉草稿、另发一条。体验差一截，但起码不留垃圾。
