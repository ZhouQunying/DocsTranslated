...
并行专家队列让一个 Gateway 把不同的聊天或房间路由到不同 agent，同时保持用户体验快速。诀窍是把并行当作"稀缺资源设计问题"来处理，而不是简单地"加更多 agent"。
...
OpenClaw 已经按会话串行运行，并通过 [命令队列](/concepts/queue) 限制全局并行。专家队列在这之上加策略：哪个 agent 拥有哪份工作、什么留在聊天里、什么变成后台工作。
...
给每条队列在它的工作区和系统提示词里写一份契约：

- **职责**：这条队列拥有的工作。
- **非目标**：它应该转交、不应自己做的工作。
- **聊天预算**：快速答复留在聊天里；长任务先简短确认，然后丢到后台 sub-agent 或任务里跑。
- **转交规则**：工作归别的队列时，说清楚该去哪里，并给一份紧凑的转交摘要。
...
这是成本最低的阶段，能解决大部分堵塞：一项编码任务不再把研究队列拖成泥潭，每个聊天保持自己的上下文干净。
