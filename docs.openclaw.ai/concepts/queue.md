...
### 10 个用户同时发消息，Agent 会怎样？

如果不做任何控制：10 个并发的 LLM 调用同时跑，同时读写同一份会话文件，API 限速被打爆，Agent 可能同时接受了两条相互矛盾的指令。说白了——并发一上来就炸。

### 解法的直觉：操作系统的进程调度

OS 不会让所有进程同时抢 CPU。它有调度器，按优先级和时间片排队。OpenClaw 搬了同一招，做了两级排队：

**第一级：会话锁**。同一个会话同时只能有一个 Agent 运行。张三和李四可以并行，但张三连发三条只能排着来。这防的是"自己跟自己打架"。

**第二级：全局并行度上限**。所有会话的运行再统一排进 `main` 队列，`maxConcurrent` 卡总并行数。防的是"LLM 调用把 API 配额打爆"。

后台任务（定时 cron、子 Agent）有自己独立的队列，不跟用户实时消息抢位子。

### 关键设计选择：用户发了新消息但 Agent 正在跑，怎么办？

这才是整篇文档最有意思的部分。四种策略，对应四种完全不同的产品语义：

- **`steer`（默认）**：把新消息塞进正在跑的上下文里。像你对正在写代码的同事喊一句"对了，还有个需求"——他不用重头开始，直接吸收新信息继续干。
- **`followup`**：排队等。当前这条跑完了再处理下一条。适合每条消息需要独立完整回复的场景。
- **`collect`**：攒一批。等消息不来了，打包成一条一起处理。适合用户习惯连发好几条的场景。
- **`interrupt`**：杀掉当前的，跑最新的。用户改主意了，之前的不要了。

为什么不只做 `followup` 就完事？因为 `steer` 在对话场景里体验远好于排队——用户补充一句话不该触发整个新的 LLM 调用。但 `steer` 实现复杂得多，需要在 tool call 执行边界注入，不能打断正在跑的工具。

### 一个 UX 小招：立刻发"输入中"

用户消息进队列的瞬间就发 typing indicator，哪怕 Agent 还在排队等。用户感知是"它在想了"，实际可能还没轮到它。感知延迟和实际延迟是两回事——这一招零成本但体验提升明显。
