...
### Agent 正在跑，用户又发了一句话——塞到哪里？

这是 queue.md 那个"四种模式"里 `steer` 的具体实现细节。问题很具体：Agent 正在执行工具调用（比如写文件），用户补了一句"对了放 /tmp 下"。这句话什么时候注入上下文？

**不能打断正在跑的工具**。工具有副作用——文件写到一半被打断就是脏数据。也不能在工具调用和它的结果之间插入——模型需要看到完整的"我请求了 X → X 返回了 Y"对。

所以注入点只有一个安全窗口：**所有工具调用执行完、下一次调模型之前**。这时候把排队消息追加为 user messages，模型下次调用就能看到。

### 为什么默认选"尽快注入"而不是"排队等"？

取舍很清楚：

- 尽快注入（steer）= 响应性好，但 Agent 可能被中途改变方向搞混
- 排队等（followup）= 连贯性好，但用户补充个信息都得等一整轮跑完

实际场景里，用户中途发的大多是补充信息而非矛盾指令。所以默认 steer——赌的是用户在帮忙而不是在捣乱。赌输了（真的矛盾了），还有 `interrupt` 模式兜底。
