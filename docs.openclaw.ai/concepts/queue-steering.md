...
会话运行已经在流式时常规 prompt 到达，队列模式是 `steer` 时 OpenClaw 默认尝试把这个 prompt 送进活跃 runtime。这个默认行为不需要配置或队列指令。Pi 和原生 Codex app-server harness 在投递细节上各不相同。
...
steering 不会中断已经在跑的工具调用。Pi 在模型边界检查排队的 steering 消息：
...
1. assistant 请求工具调用。
2. Pi 执行当前 assistant 消息的工具调用批次。
3. Pi 发出 turn end 事件。
4. Pi 消费排队的 steering 消息。
5. 在下一次 LLM 调用之前，Pi 把这些消息作为 user 消息追加上去。
...
原生 Codex app-server harness 暴露的是 `turn/steer`，而不是 Pi 内部的 steering 队列。OpenClaw 在配置的静默窗口内合并排队 prompt，然后按到达顺序把所有收集到的用户输入打包成一次 `turn/steer` 请求。
...
>
> * With default behavior, the active runtime receives all four messages in arrival order before its next model decision. Pi drains them at the next model boundary; Codex receives them as one batched `turn/steer`.
...
- 默认行为：活跃 runtime 在它下一次模型决策之前按到达顺序收到四条消息。Pi 在下一个模型边界消费；Codex 把它们作为一条合并的 `turn/steer` 收到。
...
`messages.queue.debounceMs` 作用于 `followup` 和 `collect` 的排队投递。在原生 Codex harness 的 `steer` 模式下，它还设置发送合并 `turn/steer` 前的静默窗口。Pi 的活跃 steering 本身不用防抖定时器 ——Pi 自然地把消息缓冲到下一个模型边界。
