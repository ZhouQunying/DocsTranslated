...
Agent 循环对模型的上下文理解能力、推理速度和工具调用准确性要求极高。单个 24 GB GPU 只能处理轻量级 prompt 且延迟显著增加。推荐至少两台满配 Mac Studio 或等价 GPU 设置（约 $30k+）才能保证流畅的 agent 交互。始终运行硬件能支持的最大全尺寸模型。更小或大量量化的模型不仅推理能力下降，还会增加安全风险。
