...
### Exec tool 的双模式设计——为什么分 foreground 和 background？

exec tool 提供配置选项和操作行为，分两种执行模式：

- **foreground**：同步执行，等待命令完成并返回结果
- **background**：异步执行，立即返回 session ID，后续用 process tool 查询
...
当 exec tool 创建的任务又创建了子进程（比如 shell 脚本调用其他程序），bridging 机制确保 signal 正确转发：
...
### Process tool 的交互能力——为什么需要 send input？
...
这跟 `kubectl exec` 进入运行中的 pod 是一个思路——无需重启即可与正在运行的任务交互。JSON payload 让 agent 能结构化地解析任务输出。
