...
**洞察**: 可发布的 lockfile = 用户安装时能读取锁定的版本。

**权衡**:
- ✓ 一致: 用户安装版本 = 发布时锁定版本
- ✗ 文件大: lockfile 增加 npm 包大小

**模式**: Docker image digest——`node@sha256:abc123` 总是相同 image。

### 依赖锁定的安全意义

**问题**: 依赖升级可能引入 bug 或恶意代码?

**方案**: Lockfile 锁定版本:
- ✓ 防止 supply chain attack (供应链攻击)
- ✓ 可审计: 检查所有依赖
- ✓ 可复现: 任何时候安装都相同

**洞察**: 供应链攻击 = 依赖被入侵,lockfile 锁定旧版本,不自动升级。
