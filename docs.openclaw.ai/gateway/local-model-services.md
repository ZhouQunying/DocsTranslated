...
进程租约机制确保正在处理流式响应的进程不会被提前终止。只有当 response body 处理完成后，idle 计时器才开始计时。可以类比 HTTP 服务器的 graceful shutdown：不是立即终止（kill）进程，而是等待当前请求完成。模型推理服务冷启动可能需要数十秒甚至数分钟，意外终止后重启的代价远大于多等几秒。

### 并发启动串行化——为什么需要防止重复启动？

多个模型请求同时到达且指向同一个后端配置时，没有串行化控制就会重复启动多个服务器进程，浪费 GPU 资源并导致端口冲突。系统按 provider 命令 + args 组合创建互斥锁，确保同一配置只启动一个服务器。这类似于单例模式的进程级实现——第一次请求创建实例，后续请求共享。串行化只影响启动操作，不影响已启动服务器的并发请求处理。
