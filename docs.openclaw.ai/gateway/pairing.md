...
### Token 轮换策略——为什么每次重新配对都生成新 token？

批准时颁发新 token，每次重新配对都轮换：
...
这跟 OAuth2 refresh token rotation 是一个思路——每次使用都生成新 token，旧 token 立即失效。如果 token 泄露，下一次重新配对后旧 token 自动作废。
...
### Node command gating——为什么批准前禁用所有命令？

Node commands 在配对批准前保持禁用，排队的命令被丢弃而非延迟：
...
### 本地判定的双重验证——为什么要同时检查 socket 和 proxy？

Loopback 判定需要 raw socket 来源和上游 proxy header 都确认为本地连接。如果两者不一致，取消 loopback 声明，需要显式批准。

这跟双因素认证是一个思路——单一证据不可靠，需要两个独立来源一致才能确认。防止通过伪造 proxy header 来冒充本地连接。
