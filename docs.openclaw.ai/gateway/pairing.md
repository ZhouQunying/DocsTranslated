...
### 网关作为唯一权威——为什么 CLI 不能决定成员资格？

网关主导的配对把网关作为节点成员资格决策的唯一权威来源。CLI 和 UI 工具只是审批界面，不是权威来源。

这跟 K8s 的 apiserver 是一个思路——apiserver 是集群状态的唯一权威，kubectl 只是操作界面。多个工具（CLI、macOS 应用、未来前端）都能触发审批，但最终的成员资格决策由网关统一执行。集中权威防止多工具间的权限不一致。
