...
这跟 `terraform validate` 是一个思路——校验配置但不 apply。CI 需要确保"当前配置是健康的"，但不想在 CI 流水线中修改任何东西。
