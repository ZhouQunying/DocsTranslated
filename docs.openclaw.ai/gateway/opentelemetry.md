...
1. **Counter/Histogram metric**：token 消耗、消息路由、语音交互、队列深度、内存使用
2. **Span trace**：模型调用、测试框架生命周期、工具执行的调用链
3. **Structured log record**：诊断事件的详细记录

这跟 OpenTelemetry 的标准信号分类是一个思路——metric 回答"发生了什么"（量），trace 回答"怎么发生的"（链路），log 回答"为什么发生"（详情）。三种信号互补，不是互相替代。

### Privacy——为什么 prompt/tool payload 是 opt-in？

OTEL 导出自动脱敏对话文本，prompt 和工具 payload 是按需开启（需要显式切换）。默认不发送。

这跟 Datadog APM 的 trace redaction 是一个思路——默认自动脱敏 HTTP header/payload，敏感数据按需开启发送。防止"运维开启了 OTEL 但不知道对话内容被发送到后端"。
