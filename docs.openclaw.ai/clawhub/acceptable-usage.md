...
### "端到端滥用工作流" vs "孤立关键词"——实用主义审核

ClawHub 的审核哲学是关注**端到端滥用工作流**，而非孤立关键词。一个技能包含"password"这个词不代表它是密码窃取工具——可能只是密码管理助手。但如果技能的整体意图是"获取未授权访问、滥用平台、诈骗他人、侵犯隐私或启用非自愿行为"，那它就被禁止。

这跟 GitHub 的 acceptable use policy 是一个思路——GitHub 不禁止"exploit"这个词（可能是安全研究），但禁止"用于攻击未经授权的系统"的 exploit。意图和上下文比关键词重要。

代价是审核需要人工判断——不能纯靠关键词过滤器。但这是实用主义的取舍。过度过滤（禁止所有含"hack"的技能）会扼杀合法的安全研究和教育内容。过滤不足（只看关键词不看意图）会放过伪装的恶意工具。

### 明确允许的模式——给合法用途安全港

页面明确列出了允许的模式（前端工作、shadcn/ui 组合、浏览器自动化等）。这是**安全港**设计——不是所有可能的用途都需要审核，明确允许的类别给用户信心。

这跟 AWS Acceptable Use Policy 的"permitted uses"列表一样——AWS 明确允许渗透测试自己的资源，用户不需要每次测试前申请许可。明确允许减少审核摩擦，让用户知道边界在哪。
